...
ADD magento /etc/nginx/sites-available/magento
ADD supervisord.conf /etc/supervisor/conf.d/supervisord.conf

RUN php5enmod mcrypt && \
    mkdir -p /etc/nginx/sites-enabled /var/log/fpm && \
    rm -fr /etc/nginx/conf.d/* && \
    ln -sf /etc/nginx/sites-available/magento /etc/nginx/sites-enabled/magento

VOLUME ["/var/www/html", "/var/log/nginx", "/var/log/fpm"]
EXPOSE 80
